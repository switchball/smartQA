...
from .config import RAW_TEXT_DIR, ASSETS_DIR, RELOAD_FLAG
...
def get_static_resource():
    ...
    line_indexer = LineIndexer(os.path.join(ASSETS_DIR, "line_indexer.txt"))
    line_indexer.init(glob(os.path.join(RAW_TEXT_DIR, "*.txt")), text_processor.proc_file)

    vector_engine = VectorEngine(384, emb_fn=embedding_paragraph_bert_batched)
    if RELOAD_FLAG or not vector_engine.load(os.path.join(ASSETS_DIR, "vectors.ann")):
